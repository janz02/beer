...
Currently the `hardResetStore` function can be found here, that is called during login and logout to purge the current states and routing history.
...
The DevTools middleware can be disabled in the [store.ts](store.ts) file by:
