...
export const CategoryList: FC<CategoryListProps> = props => {
  ...
  const editedId = useSelector((state: RootState) => state.categoryEditor.category?.id)
    ...
    () => [
      {
        title: t('common.data'),
        key: 'data',
        render(_value: any, record: Category, _index: number) {
          return (
            <>
              <h4>{record.name}</h4>
            </>
  ...
  const paginationConfig = useMemo(
    (): TablePaginationConfig => ({
      showTotal: (total, range) => `${range[1] - range[0] + +!!range[1]} / ${total}`,
      simple: isMobile,
  ...
  const errorMsg = (): JSX.Element => <div className="list-error">{error}</div>
  ...
  return (
    <>
      <Table
        ...
        rowClassName={row => (row.id === editedId ? 'category-list__item--editing' : '')}
      />
      <CategoryDeletePopup
