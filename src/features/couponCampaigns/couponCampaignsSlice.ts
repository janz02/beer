...
interface CouponCampaignsState {
...
const initialState: CouponCampaignsState = {
  featureState: FeatureState.Initial
}

const couponCampaignsSlice = createSlice({
  ...
  reducers: {
    ...
    setCoupon(state, action: PayloadAction<CouponCampaign | undefined>) {
      state.coupon = action.payload
    },
    getCouponSuccess(state, action: PayloadAction<CouponCampaign>) {
      state.coupon = action.payload
    ...
    getCouponCommentsSuccess(state, action: PayloadAction<CouponCampaignComment[]>) {
      if (state.coupon) {
...
const createCoupon = (coupon: CouponCampaign): AppThunk => async dispatch => {
  dispatch(setFeatureState(FeatureState.Loading))
  ...
  try {
    ...
    history.push('/couponCampaigns')
  } catch (err) {
    dispatch(setFeatureState(FeatureState.Error))
  }
}

const updateCoupon = (coupon: CouponCampaign): AppThunk => async dispatch => {
  dispatch(setFeatureState(FeatureState.Loading))
...
const getCategories = (): AppThunk => async dispatch => {
  ...
  try {
    ...
    dispatch(
      getCategoriesSuccess(
        categories.result!.map(x => ({ id: x.id, name: x.name } as CouponCampaignCategory))
      )
    )
  } catch (err) {
    dispatch(setFeatureState(FeatureState.Error))
...
const downloadCoupons = (coupon: CouponCampaign): AppThunk => async dispatch => {
  dispatch(setFeatureState(FeatureState.Loading))
...
const downloadClaimedCoupons = (coupon: CouponCampaign): AppThunk => async dispatch => {
  dispatch(setFeatureState(FeatureState.Loading))
...
const downloadPrizeFile = (coupon: CouponCampaign): AppThunk => async dispatch => {
  dispatch(setFeatureState(FeatureState.Loading))
...
const downloadPredefinedCodesFile = (coupon: CouponCampaign): AppThunk => async dispatch => {
  dispatch(setFeatureState(FeatureState.Loading))
...
export const couponCampaignActions = {
  resetCampaigns,
  setEditing,
  setSelectedCouponType,
  setSelectedCouponMode,
...
export const couponCampaignsReducer = couponCampaignsSlice.reducer
