...
const saveProduct = (product: Product): AppThunk => async dispatch => {
  ...
  try {
    if (id && !isNaN(id)) {
      await api.products.updateProduct({ id, productModel: { id, name: product.name } })
    } else {
      const createdId = await api.products.createProduct({
        productModel: { id, name: product.name }
      })
      id = createdId
    }
