...
const authMiddlewares = [
  {
    pre: async (ctx: any) => {
      ...
      try {
        ...
        const user = await couponApi.auth.refresh({ refreshDto: { refreshToken: refreshToken } })
...
export const configApiMiddleware = (): void => {
  couponApiConfig.middleware.push(...authMiddlewares)
  campaignApiConfig.middleware.push(...authMiddlewares)
}
